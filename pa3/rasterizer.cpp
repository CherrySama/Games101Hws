...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    for (const auto& t:TriangleList)
    {
        ...
        // local -> world -> view -> perspective space
        Eigen::Vector4f v[] = {
                mvp * t->v[0],
                mvp * t->v[1],
                mvp * t->v[2]
        ...
        // 进行了MVP变换、齐次处理和视口变换的三角新坐标填入三角形newtri
        for (int i = 0; i < 3; ++i)
        {
        ...
        // 然后填入正确变换的法线向量坐标，这个坐标是在摄像机空间中的坐标，
        // 也就是进行了 model, view 变换但是没有进行 projection 变换的坐标，法线向量进行投影变换是没有意义的。
        for (int i = 0; i < 3; ++i)
        {
            //view space normal
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos)
{
    ...
    for (int i = min_x; i <= max_x; ++i)
    {
        ...
        {
            if (insideTriangle(static_cast<float>(i), static_cast<float>(j), t.v))
            {
                //    * v[i].w() is the vertex view space depth value z.
                //    * Z is interpolated view space depth for the current pixel
                //    * zp is depth between zNear and zFar, used for z-buffer

                // 计算该点的重心坐标,注意：计算的坐标是经过projection变换后的点，并不能直接应用于插值
                // 需要在计算出摄像机空间中这一点的重心坐标 α,β,γ 
