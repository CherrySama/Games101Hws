...
// tnear, u, v 需要用Moller-Trumbore算法来更新的参数。
// Möller-Trumbore算法是一种快速的射线-三角形相交测试算法，
// 它通过计算叉积和点积来确定射线是否与三角形相交，并且能够计算出交点的UV坐标。
                          ...
                          const Vector3f& dir, float& tnear, float& u, float& v)
{
    ...
    // 如果交点距离tnear小于0，表示射线在起点之前与三角形相交，这通常被认为是不相交。
    if (tnear < 0) return false;
    // 如果交点的UV坐标都在[0,1]范围内，并且它们的和小于1，表示交点在三角形内部，返回true表示相交。
