...
enum MaterialType { DIFFUSE, MICROFACET_DIFFUSE, MICROFACET_GLOSSY, MIRROR};
...
float Material::pdf(const Vector3f &wi, const Vector3f &wo, const Vector3f &N){
    switch(m_type){
        ...
        case MIRROR:
        {
            // 全镜面反射只有一个方向的光线能被眼睛接收，所以pdf就设置为1
            if (dotProduct(wo, N) > 0.0f)
                return 1.0f;
            else
                return 0.0f;
...
Vector3f Material::cookTorrance(const Vector3f &wi, const Vector3f &wo, const Vector3f &N){
    ...
    // 计算微观结构分布
    double m = (type == MaterialType::MICROFACET_DIFFUSE) ? 0.5 : 0.2; // Roughness
...
Vector3f Material::eval(const Vector3f &wi, const Vector3f &wo, const Vector3f &N){
    switch(m_type){
        ...
        case MICROFACET_DIFFUSE:
        {
            float cosalpha = dotProduct(N, wo);
            if (cosalpha > 0.0f) {
                auto ans = Ks * cookTorrance(wi, wo, N) + Kd / M_PI;
                return ans;
            }
            else
                return Vector3f(0.0f);
