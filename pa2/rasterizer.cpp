...
{   
    ...
    float judge1 = AB[0] * AP[1] - AB[1] * AP[0];
    float judge2 = BC[0] * BP[1] - BC[1] * BP[0];
    float judge3 = CA[0] * CP[1] - CA[1] * CP[0];

    if (judge1 > 0 && judge2 > 0 && judge3 > 0) return true;
    else if (judge1 < 0 && judge2 < 0 && judge3 < 0) return true;
    else return false;
}
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    // ensure the boundary of bounding box
    ...
    //MSAA
    std::vector<Eigen::Vector2f> msaaPos
    {                               
        {0.25,0.25},               
        {0.75,0.25},                
        {0.25,0.75},               
    ...
    // start to traverse bounding box
    for (int i = min_x; i <= max_x; i++)
    {
        for (int j = min_y; j <= max_y; j++)
        {
            ...
            {
                if (insideTriangle(static_cast<float>(i + msaaPos[MSAA_4][0]), static_cast<float>(j + msaaPos[MSAA_4][1]), t.v))
                {
                    ...
                    if (super_depth_buf[get_index(i,j)*4+MSAA_4] > z_interpolated)
                    {
                        passTest = true;
                        // depth_buf[get_index(i, j)] = z_interpolated; 
                        // set_pixel({static_cast<float>(i), static_cast<float>(j), z_interpolated}, t.getColor()*count/4.0f);
                        super_depth_buf[get_index(i,j)*4+MSAA_4] = z_interpolated;
                        super_frame_buf[get_index(i,j)*4+MSAA_4] = t.getColor();
                    }
                }
            }
            if (passTest)
            {
                Eigen::Vector3f point = {static_cast<float>(i), static_cast<float>(j), 1};
                Eigen::Vector3f color = (super_frame_buf[get_index(i,j)*4] + super_frame_buf[get_index(i,j)*4 + 1] + super_frame_buf[get_index(i,j)*4 + 2] + super_frame_buf[get_index(i,j)*4 + 3]) / 4.0f;
